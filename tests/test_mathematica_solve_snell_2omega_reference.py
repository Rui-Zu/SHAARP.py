...
REFERENCE_PATH = (
    ...
    / "solve_snell_2omega_reference_v1.json"
)


class MathematicaSolveSnell2OmegaReferenceTests(unittest.TestCase):
    def test_mathematica_solve_snell_2omega_reference_matches_python_values(self):
        payload = json.loads(REFERENCE_PATH.read_text(encoding="utf-8"))
        self.assertIn("Mathematica", payload["source"])
        self.assertIn("2omega", payload["source"])
        ...
        self.assertEqual(payload["selectedKeys"]["epsilon2OmegaP"], [949, 50, 969, 80])
        self.assertEqual(payload["selectedKeys"]["epsilon2OmegaL"], [949, 50, 969, 76])
        self.assertEqual(payload["selectedKeys"]["QP2Omega"], [81, 80, 50, 969])
        ...
        for case in payload["cases"]:
            with self.subTest(case=case["id"]):
                ...
                incident_index = numeric_array(inputs["incident_index"]).item()
                output_omega = numeric_array(inputs["output_omega"]).item()
                ...
                self._assert_mode_matches(case, "fast", modes.fast, output_omega)
                self._assert_mode_matches(case, "slow", modes.slow, output_omega)

    def _assert_mode_matches(self, case, key, mode, output_omega):
        ...
        np.testing.assert_allclose(
            mode.refractive_index * output_omega * mode.direction,
            expected_k,
            atol=1e-9,
            rtol=1e-9,
        )
