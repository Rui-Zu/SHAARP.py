...
REFERENCE_PATH = ROOT / "benchmarks" / "mathematica_reference" / "maker_fringes_reference_pgml2.json"
PYTHON_PATH = ROOT / "benchmarks" / "maker_fringes_benchmarks_pgml2.json"

ATOL = 1e-9
RTOL = 1e-9
EXPECTED_CASE_COUNT = 10
POINT_GROUPS = {"mm2", "4mm", "2", "m", "-4", "422", "3", "-6", "4", "6"}


@unittest.skipUnless(REFERENCE_PATH.exists(), "pgml2 live Wolfram reference not present")
class MakerFringesPointGroupAtDepthPgml2ReferenceComparisonTests(unittest.TestCase):
    """POINT-GROUP-AT-DEPTH end-to-end Maker SHG for the REMAINING 10 distinct
    patterns (mm2/4mm/2/m/-4/422/3/-6/4/6), each carrying a SYMMETRY-CONSTRAINED
    d-tensor inside a 4-layer stack (air / point-group-film / isotropic-interlayer
    / substrate). Together with pgml (3m/-42m/32/-6m2) this brings point-group-at-
    depth to FULL PARITY: every distinct buildable SHG d-pattern is now validated
    point-group-at-depth vs live Wolfram f4NL, matching the single-film (pg1+pg2+
    pg3) and SI-reflected (sipg1+sipg2) coverage."""
    ...
    def test_pgml2_grid_matches_live_mathematica(self):
