...
REFERENCE_PATH = ROOT / "benchmarks" / "mathematica_reference" / "maker_fringes_reference_ml2.json"
PYTHON_PATH = ROOT / "benchmarks" / "maker_fringes_benchmarks_ml2.json"
...
@unittest.skipUnless(REFERENCE_PATH.exists(), "ml2 live Wolfram reference not yet exported")
class MakerFringesMultilayerMl2ReferenceComparisonTests(unittest.TestCase):
    """5-LAYER SHAARP.ml Maker SHG breadth validation: a stack
    air / active-film / ANISOTROPIC-interlayer / isotropic-interlayer / substrate,
    incidence-swept, validated value-by-value against live Wolfram 14.3 f4NL.

    Exercises birefringent (non-degenerate) internal-layer propagation + an extra
    interface beyond the ml1 4-layer grid. See the project notes
    ' Multilayer Breadth: 5-Layer ml2'."""
    ...
    def test_reference_is_live_wolfram_sourced_5_layer(self):
    ...
    def test_ml2_grid_matches_live_mathematica(self):
