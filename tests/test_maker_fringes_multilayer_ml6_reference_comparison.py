...
REFERENCE_PATH = ROOT / "benchmarks" / "mathematica_reference" / "maker_fringes_reference_ml6.json"
PYTHON_PATH = ROOT / "benchmarks" / "maker_fringes_benchmarks_ml6.json"
...
@unittest.skipUnless(REFERENCE_PATH.exists(), "ml6 live Wolfram reference not yet exported")
class MakerFringesMultilayerMl6ReferenceComparisonTests(unittest.TestCase):
    """7-LAYER SHAARP.ml Maker SHG depth validation: a stack
    air / active-film / ANISOTROPIC-interlayer / isotropic-interlayer /
    isotropic-interlayer-2 / isotropic-interlayer-3 / substrate, incidence-swept,
    validated value-by-value against live Wolfram 14.3 f4NL.

    Extends the ml5 6-layer grid one internal layer deeper (a fifth internal
    layer + a fourth internal interface, with three distinct isotropic
    interlayers). Only the film is SHG-active. See the project notes
    ' Multilayer Depth: 7-Layer ml6'."""
    ...
    def test_reference_is_live_wolfram_sourced_7_layer(self):
        ...
        for case in self.python["cases"]:
            self.assertEqual(case["layer_count"], 7)

    def test_ml6_grid_matches_live_mathematica(self):
