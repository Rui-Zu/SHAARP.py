...
REFERENCE_PATH = ROOT / "benchmarks" / "mathematica_reference" / "maker_fringes_reference_ml7.json"
PYTHON_PATH = ROOT / "benchmarks" / "maker_fringes_benchmarks_ml7.json"
...
@unittest.skipUnless(REFERENCE_PATH.exists(), "ml7 live Wolfram reference not yet exported")
class MakerFringesMultilayerMl7ReferenceComparisonTests(unittest.TestCase):
    """8-LAYER SHAARP.ml Maker SHG depth validation: a stack
    ...
    substrate, incidence-swept, validated value-by-value against live
    Wolfram 14.3 f4NL.

    Extends the ml6 7-layer grid one internal layer deeper (a sixth internal
    layer + a fifth internal interface, with five distinct passive interlayers:
    two anisotropic + three isotropic). Only the film is SHG-active. See
    the project notes ' Multilayer Depth: 8-Layer ml7'."""
    ...
    def test_reference_is_live_wolfram_sourced_8_layer(self):
        ...
        for case in self.python["cases"]:
            self.assertEqual(case["layer_count"], 8)

    def test_ml7_grid_matches_live_mathematica(self):
