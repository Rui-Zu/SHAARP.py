...
class JKHHMultilayerAgreementTests(unittest.TestCase):
    def _run(self, mrassumption: int) -> np.ndarray:
        ...
        # FIELD basis: bare |E|^2, the convention SHAARP.ml's MFList emits. The physical intensity
        # (list_mf_para) additionally carries the exit medium's index at 2omega, so it agrees with
        # the original only for an air substrate -- this case is not one.
        arr = np.array(sweep.shaarp_ml_copy_lists_field_basis()[0])
