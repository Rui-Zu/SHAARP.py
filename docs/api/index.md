...
The public surface of the `shaarp` package, grouped by role. Most of it is importable directly from
`shaarp` (e.g. `from shaarp import run_si_numeric`). Three groups live in their own modules: the
case-study materials in `shaarp.casestudy_materials`, the index-table functions in
`shaarp.dispersion`, and the spectrum factories in `shaarp.spectral` (e.g.
...
|---|---|---|
| **Facade** | `shaarp.api` | `run_si_numeric`, `run_ml_numeric`, `run_maker_fringes`, `run_fresnel_sweep`, the two `*_analytical` entry points, and the three wavelength sweeps. Assembles inputs, picks a workflow, wraps the answer in a `SHAARPResult`. This is what the GUI's **Update** calls. |
...
```{note}
`shaarp.__all__` exports ~160 names, most of them solver internals reached through the facades. The
...
`run_fresnel_sweep`, `run_si_full_analytical`, `run_ml_partial_analytical`, the wavelength sweeps
`run_si_spectrum` / `run_ml_spectrum` / `run_spectral_map`, plus the high-level
