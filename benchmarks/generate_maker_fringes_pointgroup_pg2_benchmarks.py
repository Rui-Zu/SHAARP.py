"""Generate a POINT-GROUP Maker-fringe benchmark grid (pg2).

This extends pg1 (3m / -42m / mm2 / 4mm) to broader point-group coverage of the
end-to-end Maker pipeline. pg2 validates the full incidence-swept Maker
end-to-end pipeline driven by SYMMETRY-CONSTRAINED point-group d-tensors for
eight additional crystal classes -- 2, m, -4, 422, 3, 32, -6, -6m2 -- each with a
DISTINCT symmetry-constrained d-tensor pattern, against live Wolfram f4NL. This
is the same physical axis as pg1, just broader crystallographic coverage; it is
distinct from the depth axis (ml1-ml8) and from the isolated symbolic doldExp
validation.
...
# Distinct build_index pool + cycling orientation_index (0-3) + varied polarimetry.
RAW_CASES = [
    ("2", 1, 0, 30.0, 45.0, 8.0),
    ("m", 4, 1, 45.0, 30.0, -12.0),
    ("-4", 8, 2, 60.0, 15.0, 20.0),
    ("422", 11, 3, 25.0, 55.0, -22.0),
    ("3", 15, 0, 40.0, 35.0, 14.0),
    ("32", 18, 1, 50.0, 20.0, -6.0),
    ("-6", 2, 2, 35.0, 50.0, 25.0),
    ("-6m2", 9, 3, 55.0, 25.0, -18.0),
]
...
def build_pg2_cases(limit: int | None = None) -> list[dict]:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate point-group SHAARP.ml Maker benchmarks + Mathematica inputs (pg2).")
    ...
    pg_cases = build_pg2_cases(limit=args.limit)
    ...
    payload = {
        ...
        "notes": [
            "Point-group SHG: the active film carries a SYMMETRY-CONSTRAINED d-tensor (2/m/-4/422/3/32/-6/-6m2), not a generic 18/18 tensor.",
            "Extends pg1 (3m/-42m/mm2/4mm) to broader crystal-class coverage of the incidence-swept Maker end-to-end pipeline vs live Wolfram f4NL.",
        ],
        "cases": records,
    }
    ...
    input_payload = {
        "benchmark_version": BENCHMARK_VERSION,
        "tag": TAG,
        "source": "Python explicit point-group Maker input manifest (pg2) for Mathematica SHAARP.ml MF export",
        "status": "python_input_manifest_not_mathematica_validation",
        "case_count": len(input_cases),
        "theta_sweep_deg": THETA_SWEEP_DEG,
        "notes": [
            "Active films carry symmetry-constrained point-group d-tensors (2/m/-4/422/3/32/-6/-6m2).",
    ...
    print(f"Wrote {len(records)} pg2 point-group cases to {args.output}")
    print(f"Wrote pg2 Mathematica inputs to {args.mathematica_input}")
