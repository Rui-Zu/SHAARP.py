"""Generate a 9-LAYER SHAARP.ml Maker benchmark grid (ml8) for depth.

ml1 (4-layer), ml2 (5-layer), ml3 (two-active 4-layer), ml5 (6-layer), ml6
(7-layer), and ml7 (8-layer) all live-Wolfram-validated the single-active
multilayer stack at increasing depth. ml8 extends that depth ONE MORE layer:
  ...
  interlayer-2 / isotropic interlayer-4 / substrate
(9 layers, 7 internal). This is ml7's 8-layer stack plus ONE MORE passive
interlayer (a FOURTH isotropic, non-degenerate interlayer placed just before
the substrate), giving SIX distinct passive interlayers between film and
substrate: 2 anisotropic + 4 isotropic, all genuinely distinct media. Only the
film is SHG-active; all passive layers carry a zero SHG d-tensor, so the active
film remains the only 2omega source (no multi-active-source coupling -- that is
ml3's job). Incidence-swept.

Reuses ml2's _anisotropic_interlayer + _isotropic_interlayer, ml5's
_isotropic_interlayer_2, ml6's _isotropic_interlayer_3, and ml7's
_anisotropic_interlayer_2 helpers, and adds ONE more passive isotropic
interlayer (_isotropic_interlayer_4) with a fresh distinct seed offset and
thickness so all six interlayers differ. Plus the existing Maker comparator +
exporter (path-override wrapper); the Wolfram side computes
MFList/listMFpara/listMFperp from SHAARP.ml f4NL. The base exporter's
makeMaterial /@ case["layers"] and f4NL handle any layer count, so no exporter
changes are needed. The fourth isotropic interlayer uses distinct deterministic
indices and thickness so it does not degenerate with any prior isotropic
interlayer.
"""
...
def _isotropic_interlayer_4(build_index: int) -> Layer:
    """A FOURTH deterministic isotropic passive (zero-SHG) interlayer.

    Mirrors ml2's _isotropic_interlayer, ml5's _isotropic_interlayer_2, and
    ml6's _isotropic_interlayer_3 but with yet-different index offsets and a
    different thickness so this seventh internal medium is genuinely distinct
    from ALL three prior isotropic interlayers (no accidental degeneracy with
    any of them)."""
    n_w = 1.38 + 0.024 * (build_index % 5)
    n_2w = 1.52 + 0.011 * (build_index % 4)
    mat = Material(
        name="interlayer-iso-4",
        structure=CrystalStructure(point_group="1"),
        orientation=CrystalOrientation(),
        epsilon_omega=np.eye(3) * (n_w ** 2),
        epsilon_2omega=np.eye(3) * (n_2w ** 2 + 0.012j),
        d_voigt_pm_v=np.zeros((3, 6), dtype=complex),
    )
    thickness = 0.05 + 0.014 * (build_index % 5)
    return Layer("interlayer-iso-4", mat, thickness_um=thickness, shg_active=False)


def build_ml8_cases(limit: int | None = None) -> list[dict]:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate 9-layer SHAARP.ml Maker benchmarks + Mathematica inputs (ml8).")
    ...
    ml_cases = build_ml8_cases(limit=args.limit)
    ...
    payload = {
        ...
        "notes": [
            "9-layer depth: ml7's 8-layer stack plus ONE MORE passive interlayer (a FOURTH non-degenerate isotropic interlayer just before the substrate), giving six distinct passive interlayers between film and substrate: 2 anisotropic + 4 isotropic.",
            "Exercises a seventh internal layer and a sixth internal interface beyond the ml7 8-layer grid.",
    ...
    input_payload = {
        "benchmark_version": BENCHMARK_VERSION,
        "tag": TAG,
        "source": "Python explicit 9-layer Maker input manifest (ml8) for Mathematica SHAARP.ml MF export",
        "status": "python_input_manifest_not_mathematica_validation",
        "case_count": len(input_cases),
        "theta_sweep_deg": THETA_SWEEP_DEG,
        "notes": [
            "9-layer stacks (air / active-film / anisotropic-interlayer / isotropic-interlayer / isotropic-interlayer-2 / isotropic-interlayer-3 / anisotropic-interlayer-2 / isotropic-interlayer-4 / substrate).",
    ...
    print(f"Wrote {len(records)} ml8 (9-layer) cases to {args.output}")
    print(f"Wrote ml8 Mathematica inputs to {args.mathematica_input}")
