...
outputPath = SHAARPPaths`Ref["shaarp_ml_definition_probe_v1.json"];

definitionText[sym_] := Quiet @ Check[ToString[Definition[sym], InputForm], "definition_probe_failed"];
downValueText[sym_] := Quiet @ Check[ToString[DownValues[sym], InputForm], "downvalues_probe_failed"];
nameProbe[pattern_String] := Sort[Names[pattern]];

Export[
  outputPath,
  <|
    "source" -> "Mathematica SHAARP.ml setup.nb loaded definitions probe",
    "status" -> "definition_probe_exported",
    "wolfram_version" -> $Version,
    "input_cell_count" -> SHAARPReferenceLoader`inputCellCount,
    "snell_names" -> nameProbe["*Snell*"],
    "wave_names" -> nameProbe["*Wave*"],
    "fresnel_names" -> nameProbe["*Fresnel*"],
    "solveSnell_definition" -> definitionText[solveSnell],
    "solveFresnelN_definition" -> definitionText[solveFresnelN],
    "f4L_definition" -> definitionText[f4L],
    "setwMed_definition" -> definitionText[setwMed],
    "setwSub_definition" -> definitionText[setwSub],
    "setWave_definition" -> definitionText[setWave],
    "extWave_definition" -> definitionText[extWave],
    "extMater_definition" -> definitionText[extMater],
    "QC2QP_definition" -> definitionText[QC2QP]
  |>,
  "JSON"
];
Quit[]
