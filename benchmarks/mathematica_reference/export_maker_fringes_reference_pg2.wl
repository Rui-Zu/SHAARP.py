...
(* Drive the existing Maker exporter against the pg2 point-group manifest. The
   exporter uses the manifest's explicit d_voigt_crystal (symmetry-constrained per
   point group); pg string only sets the nonlinear flag. All ORIGINAL-tree paths. *)
SHAARPPathOverrides`MakerInputPath = SHAARPPaths`Ref["maker_mathematica_inputs_pg2.json"];
SHAARPPathOverrides`MakerOutputPath = SHAARPPaths`Ref["maker_fringes_reference_pg2.json"];
SHAARPPathOverrides`MakerDiagnosticPath = SHAARPPaths`Ref["maker_fringes_reference_pg2_diagnostics.txt"];
