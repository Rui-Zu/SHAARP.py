...
outputPath = SHAARPPaths`Ref["shaarp_si_fresnel_stage_reference_v1.json"];
...
maxAbsOrMissing[x_] := Module[{n = Quiet[Check[N[Flatten[x]], $Failed]]},
  If[n === $Failed || !VectorQ[n, NumericQ], Missing["NonNumericResidual"], Max[Abs[n]]]
];
...
caseRecord[case_] := Module[
  {vars, ok, residuals, maxResidual, ewrootsRuleRecords},
  Clear[
  ...
  ok = And[ok, evalSnippet["omega_fresnel_substitution_active_path"]];
  residuals = Quiet[N[{
  ...
  maxResidual = maxAbsOrMissing[residuals];
  <|
    "id" -> case["id"],
    ...
    "outputs" -> <|
      ...
      "ERpNum" -> jsonValue[ERpNum],
      "ERsNum" -> jsonValue[ERsNum],
      "twe" -> jsonValue[twe],
      "two" -> jsonValue[two],
      "ETweo" -> jsonValue[ETweo],
      "ETwo" -> jsonValue[ETwo],
      "Vktweo" -> jsonValue[Vktweo],
      "Vktwo" -> jsonValue[Vktwo],
      "EIwNum" -> jsonValue[EIwNum],
      "ERwNum" -> jsonValue[ERwNum],
      "HIwNum" -> jsonValue[HIwNum],
      "HRwNum" -> jsonValue[HRwNum],
      "ETwNumExt" -> jsonValue[ETwNumExt],
      "ETwNumOrd" -> jsonValue[ETwNumOrd],
      "HTwNumExt" -> jsonValue[HTwNumExt],
      "HTwNumOrd" -> jsonValue[HTwNumOrd]
    |>,
    "checks" -> <|
      "linear_boundary_residuals" -> jsonValue[residuals],
      "max_linear_boundary_abs_residual" -> If[Head[maxResidual] === Missing, ToString[maxResidual, InputForm], N[maxResidual, 17]]
    |>
  |>
];
...
numericResiduals = Select[records[[All, "checks", "max_linear_boundary_abs_residual"]], NumericQ];

Export[
  outputPath,
  <|
    "source" -> "Mathematica SHAARP.si active omega Fresnel snippet stage reference",
    "status" -> "mathematica_stage_reference_exported",
    "validation_claim" -> "fresnel_stage_only_not_full_shaarp_agreement",
    ...
    "numeric_residual_case_count" -> Length[numericResiduals],
    "max_linear_boundary_abs_residual" -> If[Length[numericResiduals] == 0, "non_numeric", Max[numericResiduals]],
    "cases" -> records
  |>,
  "JSON"
];
