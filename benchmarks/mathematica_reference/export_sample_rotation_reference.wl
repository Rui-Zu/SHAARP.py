...
outputPath = SHAARPPaths`Ref["sample_rotation_reference_v1.json"];
debugPath = SHAARPPaths`Ref["sample_rotation_reference_debug.txt"];
...
makeCase[case_Association] := Module[
  {mats, rotations, theta, delta, phi, psi, prev, rows, totalmats},
...
outputPayload = <|
    "source" -> "Mathematica SHAARP.ml setup.nb SampleRotate reference values",
