...
inputPath = SHAARPPaths`Ref["sample_rotation_mathematica_inputs_ext1.json"];
outputPath = SHAARPPaths`Ref["sample_rotation_reference_ext1.json"];
debugPath = SHAARPPaths`Ref["sample_rotation_reference_ext1_debug.txt"];
inputPayload = Import[inputPath, "RawJSON"];
Print["Loaded SampleRotate ext input cases: ", Length[inputPayload["cases"]]];
...
safeSampleRotate[mats_, thetaDeg_, rotateDeg_, rotateStepDeg_, assumption_, totalmats_, delta_, phi_, psi_] := Module[
...
makeCase[case_Association] := Module[
  ...
  rows = Table[
    Module[{rot = rotations[[idx]], step},
      ...
      safeSampleRotate[mats, theta, rot, step, case["mrassumption"], totalmats, delta, phi, psi]
    ],
...
Print["Computed SampleRotate ext cases: ", Length[cases]];
Export[debugPath, ToString[cases, InputForm], "Text"];
Print["Wrote SampleRotate ext debug expression: ", debugPath];

outputPayload = <|
    "source" -> "Mathematica SHAARP.ml setup.nb SampleRotate ext1 reference values",
...
If[exportResult === $Failed,
  Print["SampleRotate ext JSON export failed for: ", outputPath];
  Quit[2]
];
Print["Wrote SampleRotate ext Mathematica reference JSON: ", outputPath];
Quit[]
