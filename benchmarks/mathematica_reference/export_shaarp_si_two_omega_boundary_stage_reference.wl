...
outputPath = SHAARPPaths`Ref["shaarp_si_two_omega_boundary_stage_reference_v1.json"];
...
caseRecord[case_] := Module[
  {
    ...
    maxSHBoundaryResidual, maxSHSignal
  },
  Clear[
  ...
  <|
    ...
    "policy" -> <|
      ...
      "boundary_unknown_policy" -> "ER2wp and ER2ws are solved by SHAARP.si two_omega_boundary_solution in this stage"
    |>,
    "outputs" -> <|
...
Export[
  outputPath,
  <|
    "source" -> "Mathematica SHAARP.si numerical 2omega boundary-solve snippet stage reference",
    "status" -> "mathematica_stage_reference_exported",
    "validation_claim" -> "two_omega_boundary_stage_only_not_full_shaarp_agreement",
