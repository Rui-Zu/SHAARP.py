...
Export[
  SHAARPPaths`Ref["shaarp_ml_function_definitions.json"],
  <|
    "source" -> "Mathematica SHAARP.ml setup.nb function definitions",
    "version" -> $Version,
    "inputCellCount" -> SHAARPReferenceLoader`inputCellCount,
    "definitions" -> <|
      "computePNL" -> ToString[DownValues[computePNL], InputForm],
      "solveInhom" -> ToString[DownValues[solveInhom], InputForm],
      "solveSnell" -> ToString[DownValues[solveSnell], InputForm]
    |>
  |>,
  "JSON"
];
