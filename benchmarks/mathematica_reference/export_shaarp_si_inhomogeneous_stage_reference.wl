...
outputPath = SHAARPPaths`Ref["shaarp_si_inhomogeneous_stage_reference_v1.json"];
...
caseRecord[case_] := Module[
  {vars, dInput, ok, ewrootsRuleRecords, boundaryResiduals, maxBoundaryResidual, maxPNL, crootsRuleRecords, cResiduals, maxCResidual},
  Clear[
...
Export[
  outputPath,
  <|
    "source" -> "Mathematica SHAARP.si inhomogeneous particular-solution snippet stage reference",
    "status" -> "mathematica_stage_reference_exported",
    "validation_claim" -> "inhomogeneous_stage_only_not_full_shaarp_agreement",
