...
names = Names["Global`*"];
withDown = Select[names, (Length[DownValues[#]] > 0) &];
inventory = Table[
  <|
    "name" -> n,
    "downvalue_count" -> Length[DownValues[Symbol[n]]],
    "ownvalue_count" -> Length[OwnValues[Symbol[n]]]
  |>,
  {n, withDown}
];

(* Full definitions for the candidate high-level entry points *)
wanted = {"Fresnel", "Maker", "solveFresnel", "solveFresnelN", "computePNL", "solveInhom", "solveSnell", "hklConvert"};
defs = Association @@ Table[
  n -> If[NameQ["Global`" <> n],
    StringTake[ToString[DownValues[Symbol["Global`" <> n]], InputForm], UpTo[40000]],
    "UNDEFINED"],
  {n, wanted}
];

Export[
  SHAARPPaths`Ref["shaarp_ml_symbol_inventory.json"],
  <|
    "source" -> "Mathematica SHAARP.ml setup.nb symbol inventory (all Global symbols with DownValues)",
    "wolfram_version" -> $Version,
    "inputCellCount" -> Length[cells],
    "symbols_with_downvalues" -> inventory,
    "selected_definitions_truncated_40k" -> defs
  |>,
  "JSON"
];
Quit[]
