...
(* Drive the existing Maker-fringe exporter against the ml5 MULTILAYER manifest
   (6-layer stacks: air / active-film / anisotropic-interlayer / isotropic-
   interlayer-2 / isotropic-interlayer-3 / substrate) by setting
...
SHAARPPathOverrides`MakerInputPath = SHAARPPaths`Ref["maker_mathematica_inputs_ml5.json"];
SHAARPPathOverrides`MakerOutputPath = SHAARPPaths`Ref["maker_fringes_reference_ml5.json"];
SHAARPPathOverrides`MakerDiagnosticPath = SHAARPPaths`Ref["maker_fringes_reference_ml5_diagnostics.txt"];
