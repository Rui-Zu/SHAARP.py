...
outputPath = SHAARPPaths`Ref["shaarp_si_two_omega_setup_stage_reference_v1.json"];
...
caseRecord[case_] := Module[
  ...
  <|
    ...
    "policy" -> <|
      ...
      "boundary_unknown_policy" -> "ER2wp and ER2ws remain unresolved reflected-SHG boundary unknowns in this setup stage and are not exported as numeric source fields"
    |>,
    "outputs" -> <|
...
Export[
  outputPath,
  <|
    "source" -> "Mathematica SHAARP.si numerical 2omega source setup snippet stage reference",
    "status" -> "mathematica_stage_reference_exported",
    "validation_claim" -> "two_omega_setup_stage_only_not_full_shaarp_agreement",
    ...
    "policy" -> <|
      ...
      "boundary_unknown_policy" -> "This exporter intentionally omits ER2w/HR2w numeric comparison because ER2wp and ER2ws are solved in the following 2omega boundary stage"
    |>,
