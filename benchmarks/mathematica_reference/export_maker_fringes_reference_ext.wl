...
(* Drive the existing Maker-fringe exporter against the ext1 manifest by
   setting SHAARPPathOverrides before loading export_maker_fringes_reference.wl.
   All paths are ORIGINAL-tree paths (the base exporter hardcodes that tree). *)
SHAARPPathOverrides`MakerInputPath = SHAARPPaths`Ref["maker_mathematica_inputs_ext1.json"];
SHAARPPathOverrides`MakerOutputPath = SHAARPPaths`Ref["maker_fringes_reference_ext1.json"];
SHAARPPathOverrides`MakerDiagnosticPath = SHAARPPaths`Ref["maker_fringes_reference_ext1_diagnostics.txt"];
