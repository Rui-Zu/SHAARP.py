...
(* Drive the existing Maker exporter against the pgml point-group-at-depth manifest.
   The exporter uses the manifest's explicit per-layer d_voigt_crystal (the active
   film layer carries a symmetry-constrained tensor per point group) over the 4-layer
   stack; pg string only sets the nonlinear flag. All ORIGINAL-tree paths. *)
SHAARPPathOverrides`MakerInputPath = SHAARPPaths`Ref["maker_mathematica_inputs_pgml2.json"];
SHAARPPathOverrides`MakerOutputPath = SHAARPPaths`Ref["maker_fringes_reference_pgml2.json"];
SHAARPPathOverrides`MakerDiagnosticPath = SHAARPPaths`Ref["maker_fringes_reference_pgml2_diagnostics.txt"];
