...
inputPath = SHAARPPaths`Ref["polarimetry_mathematica_inputs_v1.json"];
outputPath = SHAARPPaths`Ref["polarimetry_reference_v1.json"];
debugPath = SHAARPPaths`Ref["polarimetry_reference_v1_debug.txt"];
inputPayload = Import[inputPath, "RawJSON"];
Print["Loaded polarimetry input cases: ", Length[inputPayload["cases"]]];
...
Print["Computed polarimetry cases: ", Length[cases]];
Export[debugPath, ToString[cases, InputForm], "Text"];
Print["Wrote polarimetry debug expression: ", debugPath];

outputPayload = <|
    "source" -> "Mathematica SHAARP.ml setup.nb polarimetry (single-azimuth SampleRotate) reference values",
...
If[exportResult === $Failed,
  Print["polarimetry JSON export failed for: ", outputPath];
  Quit[2]
];
Print["Wrote polarimetry Mathematica reference JSON: ", outputPath];
Quit[]
