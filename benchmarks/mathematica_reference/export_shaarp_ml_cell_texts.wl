...
outputPath = SHAARPPaths`Ref["shaarp_ml_cell_texts.json"];
nbPath = SHAARPPaths`ML["SHAARP.ml.nb"];
nb = Import[nbPath, "NB"];
cells = Cases[nb, Cell[box_, "Input", ___] :> box, Infinity];
cellText = ToString[#, InputForm] & /@ cells;
records = Table[
  <|
    "index" -> i,
    "byte_count" -> StringLength[cellText[[i]]],
    "text" -> cellText[[i]]
  |>,
  {i, Length[cellText]}
];
Export[
  outputPath,
  <|
    "source" -> "Mathematica SHAARP.ml SHAARP.ml.nb full input cell texts",
