...
(* Drive the existing Maker-fringe exporter against the ml2 MULTILAYER manifest
   (5-layer stacks: air / active-film / anisotropic-interlayer / isotropic-
   interlayer / substrate) by setting SHAARPPathOverrides, then Get the base
   exporter. The base exporter's makeMaterial /@ case["layers"] and f4NL handle
   any layer count, so no exporter changes are needed. All paths are ORIGINAL-tree. *)
SHAARPPathOverrides`MakerInputPath = SHAARPPaths`Ref["maker_mathematica_inputs_ml2.json"];
SHAARPPathOverrides`MakerOutputPath = SHAARPPaths`Ref["maker_fringes_reference_ml2.json"];
SHAARPPathOverrides`MakerDiagnosticPath = SHAARPPaths`Ref["maker_fringes_reference_ml2_diagnostics.txt"];
