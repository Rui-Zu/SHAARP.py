...
(* Drive the existing Maker-fringe exporter against the ml1 MULTILAYER manifest
   (4-layer stacks) by setting SHAARPPathOverrides, then Get the base exporter.
   The base exporter's makeMaterial /@ case["layers"] and f4NL handle any layer
   count, so no exporter changes are needed. All paths are ORIGINAL-tree. *)
SHAARPPathOverrides`MakerInputPath = SHAARPPaths`Ref["maker_mathematica_inputs_ml1.json"];
SHAARPPathOverrides`MakerOutputPath = SHAARPPaths`Ref["maker_fringes_reference_ml1.json"];
SHAARPPathOverrides`MakerDiagnosticPath = SHAARPPaths`Ref["maker_fringes_reference_ml1_diagnostics.txt"];
