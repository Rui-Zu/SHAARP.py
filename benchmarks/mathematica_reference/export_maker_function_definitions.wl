...
definitionString[sym_] := ToString[DownValues[sym], InputForm];

Export[
  SHAARPPaths`Ref["maker_function_definitions.json"],
  <|
    "source" -> "Mathematica SHAARP.ml setup.nb Maker/Fresnel function definitions",
    "status" -> "mathematica_definition_exported",
    "wolfram_version" -> $Version,
    "inputCellCount" -> SHAARPReferenceLoader`inputCellCount,
    "definitions" -> <|
      "MF" -> definitionString[MF],
      "Fresnel" -> definitionString[Fresnel],
      "SampleRotate" -> definitionString[SampleRotate],
      "f4NL" -> definitionString[f4NL],
      "setwInc" -> definitionString[setwInc],
      "extMater" -> definitionString[extMater],
      "hklConvert" -> definitionString[hklConvert]
    |>
  |>,
  "JSON"
];
