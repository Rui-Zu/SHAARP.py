"""Generate a 7-LAYER SHAARP.ml Maker benchmark grid (ml6) for depth.

ml1 (4-layer), ml2 (5-layer), and ml5 (6-layer) all live-Wolfram-validated the
single-active multilayer stack at increasing depth. ml6 extends that depth ONE
MORE layer:
  air / active-film / ANISOTROPIC passive interlayer / isotropic interlayer /
  isotropic interlayer-2 / isotropic interlayer-3 / substrate
(7 layers, 5 internal). This is ml5's 6-layer stack plus ONE MORE isotropic
passive interlayer. The three isotropic interlayers use slightly different
deterministic indices and thicknesses so all passive media are genuinely
distinct (a fourth internal interface beyond ml5). Only the film is SHG-active;
all passive layers carry a zero SHG d-tensor, so the active film remains the only
2omega source (no multi-active-source coupling -- that is ml3's job).
Incidence-swept.

Reuses ml2's _anisotropic_interlayer + _isotropic_interlayer and ml5's
_isotropic_interlayer_2 helpers, plus the existing Maker comparator + exporter
(path-override wrapper); the Wolfram side computes MFList/listMFpara/listMFperp
from SHAARP.ml f4NL. The base exporter's makeMaterial /@ case["layers"] and f4NL
handle any layer count, so no exporter changes are needed.
"""
...
def _isotropic_interlayer_3(build_index: int) -> Layer:
    """A THIRD deterministic isotropic passive (zero-SHG) interlayer.

    Mirrors ml2's _isotropic_interlayer and ml5's _isotropic_interlayer_2 but
    with yet-different index offsets and a different thickness so this fourth
    internal medium is genuinely distinct from BOTH prior isotropic interlayers
    (no accidental degeneracy with either)."""
    n_w = 1.43 + 0.021 * (build_index % 5)
    n_2w = 1.57 + 0.009 * (build_index % 4)
    mat = Material(
        name="interlayer-iso-3",
        structure=CrystalStructure(point_group="1"),
        orientation=CrystalOrientation(),
        epsilon_omega=np.eye(3) * (n_w ** 2),
        epsilon_2omega=np.eye(3) * (n_2w ** 2 + 0.015j),
        d_voigt_pm_v=np.zeros((3, 6), dtype=complex),
    )
    thickness = 0.07 + 0.013 * (build_index % 5)
    return Layer("interlayer-iso-3", mat, thickness_um=thickness, shg_active=False)


def build_ml6_cases(limit: int | None = None) -> list[dict]:
    ...
    for build_index, orientation_index, phi_deg, psi_deg, ellipticity_deg in raw:
        ...
        system = replace(sys3, layers=[top, film, aniso, iso, iso2, iso3, substrate])
        system = replace(
            system,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate 7-layer SHAARP.ml Maker benchmarks + Mathematica inputs (ml6).")
    ...
    ml_cases = build_ml6_cases(limit=args.limit)
    ...
    payload = {
        ...
        "layer_structure": "air / active-film / anisotropic-interlayer / isotropic-interlayer / isotropic-interlayer-2 / isotropic-interlayer-3 / substrate (7 layers, 5 internal)",
        "notes": [
            "7-layer depth: ml5's 6-layer stack plus ONE MORE isotropic passive interlayer (three distinct isotropic interlayers + one anisotropic interlayer between film and substrate).",
            "Exercises a fifth internal layer and a fourth internal interface beyond the ml5 6-layer grid.",
    ...
    input_payload = {
        "benchmark_version": BENCHMARK_VERSION,
        "tag": TAG,
        "source": "Python explicit 7-layer Maker input manifest (ml6) for Mathematica SHAARP.ml MF export",
        "status": "python_input_manifest_not_mathematica_validation",
        "case_count": len(input_cases),
        "theta_sweep_deg": THETA_SWEEP_DEG,
        "notes": [
            "7-layer stacks (air / active-film / anisotropic-interlayer / isotropic-interlayer / isotropic-interlayer-2 / isotropic-interlayer-3 / substrate).",
    ...
    print(f"Wrote {len(records)} ml6 (7-layer) cases to {args.output}")
    print(f"Wrote ml6 Mathematica inputs to {args.mathematica_input}")
